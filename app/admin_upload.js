...
function validateImages(files,maxBytes){
  ...
  for(var n=0;n<list.length;n++){
    ...
    if(file.size>maxBytes)return {ok:false,message:'文件“'+file.name+'”大小 '+mib(file.size)+' MiB，超过网页单文件上限 '+mib(maxBytes)+' MiB。'};
  }
...
function validateArchive(files,maxBytes){
  ...
  if(file.size>maxBytes)return {ok:false,message:'归档“'+file.name+'”大小 '+mib(file.size)+' MiB，超过网页上限 '+mib(maxBytes)+' MiB。超大归档请改用 CLI 导入。'};
...
document.addEventListener('DOMContentLoaded',function(){
  ...
  document.addEventListener('submit',function(e){
    ...
    xhr.upload.onload=function(){status.textContent='服务器正在安全校验并生成预览';};
    xhr.onload=function(){
      ...
      setMessage(form,gateway?'上传失败（HTTP '+xhr.status+'）。可能超过网页、Cloudflare 或代理限制，请缩小归档或改用 CLI。':'归档处理失败（HTTP '+xhr.status+'），请检查文件后重试。',true);
    ...
    xhr.ontimeout=function(){setMessage(form,'上传超时，归档未完成处理；请缩小归档、检查连接或改用 CLI。',true);status.hidden=true;restore(button,original);};
