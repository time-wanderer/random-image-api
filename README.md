...
5. **删除**：删除属于高风险操作，图片库默认不显示可直接删除按钮。单图必须进入独立确认页，输入图片 ID 和 `DELETE LOCAL IMAGE`；批量删除必须进入独立确认页，核对当前筛选条件和精确数量，并输入 `DELETE LOCAL IMAGES` 及确认数量。两种操作都要求管理员会话和 CSRF，服务端会重新校验对象、范围和数量。删除图片记录时，SQLite 通过 `ON DELETE CASCADE` 清理 `image_tags` 关系，但保留标签定义；WebDAV 只允许禁用、维护标签或清理本地缓存，绝不远程删除原图。
